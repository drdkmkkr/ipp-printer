...
function getPrinterAttributes (printer, req, res) {
  var requested = utils.requestedAttributes(req._body)
...
function getJobs (printer, req, res) {
  ...
  switch (which) {
    case 'all':
      states = [C.JOB_COMPLETED, C.JOB_CANCELED, C.JOB_ABORTED, C.JOB_PENDING, C.JOB_PROCESSING, C.JOB_PROCESSING_STOPPED, C.JOB_PENDING_HELD]
      break
    case 'completed':
